...
interface PanelPaintingFormProps {
  customer: Customer;
...
export const PanelPaintingForm: React.FC<PanelPaintingFormProps> = ({ customer, onComplete }) => {
  const { 
    carParts, 
    getOptionalServices, 
  ...
  // Filter out specific parts that shouldn't be shown for panel painting
  ...
  const getBasePanelPrice = (): number => {
    ...
    // For panel painting, we get the base price from pricing table
  ...
  const calculateTotalPrice = (): number => {
    return getBasePanelPrice() + calculateAdditionalPrice();
  };

  const handleComplete = () => {
    const items: QuotationItem[] = [
      // Base panel painting item
      {
        car_part_id: 'panel_painting',
        car_part_name: 'Sơn Quây Toàn Bộ',
        selected_services: [],
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Base Panel Painting Price */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Giá sơn quây cơ bản</h3>
            <p className="text-sm text-gray-600">Bao gồm sơn toàn bộ các bộ phận xe</p>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="space-y-3 mb-4">
          <div className="flex items-center justify-between text-lg">
            <span className="text-gray-600">Giá sơn quây cơ bản:</span>
